...
if len(tickers) < 1:
    ...
    for corr in correlations:
        ...
        portfolio_volatility_modified = np.sqrt(np.dot(weights, np.dot(modified_cov_matrix, weights)))
        portfolio_volatilities.append(portfolio_volatility_modified)

    # Plot portfolio volatility vs. correlation
    plt.figure(figsize=(10, 5))
    plt.plot(correlations, portfolio_volatilities, label="Portfolio Volatility")
    plt.title("Impact of Asset Correlation on Portfolio Volatility")
    plt.xlabel("Correlation between Assets")
    plt.ylabel("Portfolio Volatility")
    st.pyplot(plt)

    # --- Short-selling Summary ---
    st.subheader("Short-Selling Summary")
    st.write(f"Your portfolio has a {100 * sum(weights < 0) / len(weights):.2f}% short position(s).")
