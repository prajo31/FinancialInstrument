...
def main():
    ...
    if user_exists.empty:
        if st.button("Submit"):
            ...
            if fv_annuity_result is not None and pv_annuity_result is not None:
                ...
                fig, ax = plt.subplots()
                ax.bar(['Future Value of Annuity', 'Present Value of Annuity'], [fv_annuity_result, pv_annuity_result],
                       color=['green', 'red'])
                ax.set_ylabel('Amount ($)')
                ax.set_title('Annuity Results Comparison')
                # Add values on top of bars
                for i, v in enumerate([fv_annuity_result, pv_annuity_result]):
                    ax.text(i, v + 0.05 * max(fv_annuity_result, pv_annuity_result), f"{v:.2f}", ha='center')
                st.pyplot(fig)
    ...
    st.markdown(
        "This table shows the impact of changes in interest rates and the number of years on PV, FV, PVA, and FVA.")

    # Create the sensitivity analysis data
    interest_rates = [0.02, 0.05, 0.10, 0.15, 0.20]
    years = [1, 5, 10, 15, 20]

    # Create a DataFrame to hold the sensitivity analysis results
    sensitivity_data = []

    for rate in interest_rates:
        for year in years:
            tvm = TVM(rate, year)
            for pv in [1000, 5000, 10000]:  # Different present values for annuities
                row = {
                    "Interest Rate (%)": rate * 100,
                    "Number of Years": year,
                    "Present Value": tvm.present_value(2000),  # Example future value of $2000
                    "Future Value": tvm.future_value(1000),  # Example present value of $1000
                    "Present Value of Annuity": tvm.present_value_annuity(pv),
                    "Future Value of Annuity": tvm.future_value_annuity(pv),
                }
    ...
    **Note:** The following sensitivity analysis is based on the following values:
